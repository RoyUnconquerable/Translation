...
Each entry is a way the English can say something the Chinese does not. The
glossary, style guide and checks resolve everything else. Retrieve only the
entries the current source makes relevant.

## Degree and certainty

Keep each distinction exactly as the source draws it:
- difficulty versus impossibility; certainty versus magnitude;
- attempt versus success; overlap versus fusion;
- understanding versus ability: knowing a method means understanding and
  using it when the scene relies on that competence, not knowing it exists;
- touching existing imagery does not create it;
- almost is not fully, and almost no hesitation is not none;
...
- His intervention and his injuries may belong to different people. Resolve
  each from the source.
- A teacher's ruler must not acquire an owner the source does not give it.
- Clarify who acts, feels, owns or serves as material without adding an
  artifact.
...
- Bare and named variants, ordinary life versus nature-and-life, and generic
  variables versus Variables follow the glossary's narrow contexts.
- Never insert a name to satisfy a scan.
...
- Ch. 1271: the rejected draft is not precedent. Its notes are in
  continuity-archive.md.
- Idioms: Reference_Idioms.md governs idiom wording. Groups 14 and 90 conflict
