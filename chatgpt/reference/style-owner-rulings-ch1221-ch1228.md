# Owner Style Rulings, Chapters 1221 to 1232
...
- Use contractions by default in narration, thought, and dialogue, but never use a contraction ending in `'d`. Write `he had`, `I would`, and similar forms in full.
...
- `Demon-Purging True Person` is a bare proper title and does not take `the` in normal narration.
- Use `Saint`, never `Sage`, for Si Sui's epithet and related references.
- Capitalize both words in `True Self` everywhere. This owner ruling overrides the older `true Self` form in the uploaded glossary.
- Owner-edited chapter prose is final. Extract reusable rulings from it rather than reverting to an earlier model draft.
